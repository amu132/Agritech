...
async function myFunction() {
    ...
    quoteBox.innerHTML = "Fetching quote...";
    authorBox.innerHTML = "Loading...";

    try {
        const response = await fetch("https://generativelanguage.googleapis.com/v1/models/gemini-pro:generateContent?key=AIzaSyDVQ9klnsH4YEHFGPNhB_uQGCaMt8rH4xw", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                contents: [{ role: "user", parts: [{ text: "Give me a quote related to agriculture and farming" }] }]
            })
        });

        const data = await response.json();
        const quote = data.candidates?.[0]?.content?.parts?.[0]?.text || "Farming looks easy when your plow is a pencil.";

        let parts = quote.split(" - ");
        quoteBox.innerHTML = parts[0];
        authorBox.innerHTML = parts.length > 1 ? parts[1] : "Unknown";
    } catch (error) {
        quoteBox.innerHTML = "Failed to fetch a quote. Try again later.";
    }
}
</script>
